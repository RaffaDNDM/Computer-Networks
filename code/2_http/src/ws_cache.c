...
int main()
{
    ...
    while( 1 )
    {
        ...
        if (!fork())
        {
            ...
            else if(!strcmp(version, "HTTP/1.0"))
            {
                for(i=0; h[i].name[0]; i++)
                {
                    if(!strcmp(h[i].name, "Connection") && !strcmp(h[i].value, "keep_alive"))
                        keep_alive=1;
                    else if(!strcmp(h[i].name, "If-Modified-Since"))
                        cache_date = h[i].value;
            ...
            { // it is a get
                ...
                else 
                {
                    if(cache_date!=NULL)
                    {
                        ...
                        strptime(cache_date, "%a, %d %b %Y %H:%M:%S %Z", &cache_tm);

                        if(timegm(&tm)>timegm(&cache_tm))
                            sprintf(response,"HTTP/1.1 200 OK\r\nConnection:close\r\n\r\n");
                        else
                        {
                            is_updated = 1;
            ...
            write(s2,response,strlen(response)); // HTTP response line
