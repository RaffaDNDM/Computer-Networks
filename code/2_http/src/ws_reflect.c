...
int main()
{
    FILE *f;
    char command[100];
    int i,s,t,s2,n,len,c,yes=1, reflect=0; 
    ...
    while( 1 )
    {
        ...
        if (!fork())
        {
            ...
            { // it is a get
                ...
                else if(!strncmp(path,"/reflect", 8))
                {
                    sprintf(response,"HTTP/1.1 200 OK\r\nConnection: close\r\n\r\n");
                    reflect=1;
                }
            ...
            else if(reflect)
            {
                ...
                *(version-1)=' ';
                request[i]='\r';
                write(s2, request, 1999); 
                unsigned char* ip_addr = (unsigned char*) &(remote.sin_addr.s_addr);
                sprintf(response, "\r\n%u.%u.%u.%u\r\n%d\r\n", ip_addr[0], ip_addr[1], ip_addr[2], ip_addr[3], ntohs(remote.sin_port));
