...
unsigned char broadcast[6] = {0x4c,0xbb,0x58,0x5f,0xb4,0xdc};
unsigned char mioip[4] = {192, 168, 1, 81};
unsigned char netmask[4] = {255, 255, 255, 0};
unsigned char gateway[4] = {192, 168, 1, 1};

//unsigned char iptarget[4] = {88,80,187,80};
unsigned char iptarget[4] = {212,71,253,5};
...
void crea_ip(struct ip_datagram *ip, int payloadsize, unsigned char proto, unsigned char *ipdest)
{
	ip->ver_ihl = 0x4F;
	ip->tos = 0;
	ip->totlen = htons(payloadsize+20+40);
	ip->id = htons(0xABCD);
   ...
   ip->option_length = 40;
   ip->option_pointer = 4;
   for(int i=0;i<37;i++) ip->route_data[i]=0;
...
int main()
{
	...
	printf("ICMP/IP/ETH\n");
	stampa_buffer(buffer, 14 + 20 + 8 + 20 +40);
	lungh = sizeof(struct sockaddr_ll);
	bzero(&sll, lungh);
	sll.sll_ifindex = if_nametoindex("wlp6s0");
	n = sendto(s, buffer, 14 + 20 + 8 + 20 + 40, 0, (struct sockaddr *)&sll, lungh);
	if (n == -1)
	{
		perror("sendto fallita");
		return 1;
